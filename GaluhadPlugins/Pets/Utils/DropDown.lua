...
LotRO Dropdown class v.2.1 By Galuhad
...
DropDown:GetSelectedIndex();				-- Gets the index of the selected item in the dropdown
...
DropDown:SetWidth(value);					-- Default width = 160
DropDown:Clear();							-- Removes all items from the menu list
DropDown:AddItem(Text);						-- Adds a text string to the menu
...
function DropDown.Constructor(sender,_list,defaultLabel)
	...
	ddLabelContainer.AddItem = function (Sender, Text)
		if Text == nil then return end;

		local index = ddListBox:GetItemCount()+1;
		...
		if tostring(Text) == tostring(defaultLabel) then
			lblItem:SetForeColor(Turbine.UI.Color.Yellow);
			ddListContainer.Selected = index;
		elseif defaultLabel == nil and index == 1 then
			lblItem:SetForeColor(Turbine.UI.Color.Yellow);
			ddListContainer.Selected = index;
		else
		...
		cItemContainer.MouseLeave = function ()
			...
			if ddListContainer.Selected == index then
				lblItem:SetForeColor(Turbine.UI.Color.Yellow);
			else
				lblItem:SetForeColor(Turbine.UI.Color((229/255),(209/255),(136/255)));
			end
		...
		cItemContainer.MouseDown = function ()
			...
			ddListContainer.Selected = index;
			lblItem:SetForeColor(Turbine.UI.Color.Yellow);
			ddLabelContainer.ItemChanged(cItemContainer,{["Text"]=Text;["Index"]=index});
		end
	...
	ddLabelContainer.Clear = function ()
		ddListBox:ClearItems();
		lblSelected:SetText("");
	end
	...
	ddLabelContainer.GetSelectedIndex = function ()
